...
class Doctor():
    ...
    def deleteDoctors(id):
        ...
        if id < 1:
            ...
            MsgBox = messagebox.askquestion("Attention!","You Want to Delete Doctor  ?",icon='warning')
            if MsgBox == 'yes':
                cur.execute ("DELETE FROM DOCTORS WHERE id=?",(id,))
                cur.execute("DELETE FROM HISTORYOFPATENTS WHERE idDoctors1=?",(id,))
                con.commit()
                messagebox.showinfo("Success","Doctor is deleted.")
                return True
    ...
    def deletePatients(id):
        ...
        if id < 1:
            ...
            if MsgBox == 'yes':
                ...
                con.commit()
                messagebox.showinfo("Success","Patient is deleted.")
                return True
